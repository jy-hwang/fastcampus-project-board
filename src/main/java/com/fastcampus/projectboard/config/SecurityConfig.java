...
public class SecurityConfig {
   ...
   *
   * <p>
   *   TODO: 카카오 도메인에 결합되어 있는코드. 확장을 고려하면 별도 인증 처리서비스 클래스로 분리하는 것이 좋지만, 현재 다른 OAuth 인증 플랫폼을 사용할 예정이 없어 이렇게 마무리한다.
   *
  ...
  public OAuth2UserService<OAuth2UserRequest, OAuth2User> oAuth2UserService(
    ...
    return userRequest -> {
      ...
      String providerId = String.valueOf(kakaoResponse.id()); // ex) 1234567890
      String username = registrationId + "_" + providerId; // ex) kakao_1234567890
      String dummyPassword = passwordEncoder.encode("{bcrypt}" + UUID.randomUUID()); // 실제 인증과정에서는 사용하지 않음.
